...
def swine_align(player_score, opponent_score):
    ...
    potential_gcd = 0
    # final_gcd = 1
    while potential_gcd <= player_score and potential_gcd <= opponent_score:
        potential_gcd += 1
        if player_score % potential_gcd == 0 and opponent_score % potential_gcd == 0:
            final_gcd = potential_gcd
...
def pig_pass(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4b
    if player_score < opponent_score and abs(player_score - opponent_score) < 3:
        return True
    return False
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if current_score - last_score > running_high:
            ...
            return announce_highest(who, current_score, diff)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def new_func(*args):
        ...
        while n <= trials_count:
            total, n = total + original_function(*args), n + 1
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    turn_score = []
    while num <= 10:
        turn_score.append(averaged_dice(num, dice))
        num += 1
    index_list = []
    for index, item in enumerate(turn_score):
        if item == max(turn_score):
            index_list.append(index)
            # index starts with 0, we need add 1 later
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # need to roll 0 dice first, and then check the number of rolls for next turn
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    if score <= 80:
        return extra_turn_strategy(score, opponent_score, cutoff=10, num_rolls=6)
    elif score <=90 and score >= opponent_score:
        return extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6)
    elif score <=90 and score < opponent_score:
        return bacon_strategy(score, opponent_score,cutoff = 5, num_rolls=6)
    else:
        return 2
